...
function EditModal({ onClose, playlist, setPlaylist, setPlaylists, isOpen, setIsOpen }) {
  ...
  // şəkil seçiləndə ölçünü kiçildib preview göstərmk
  ...
  // save basanda playlist məlumatlarını update edirəm
  ...
  // modalın boş hissəsinə klikləyəndə bağlanır
  ...
  return (
    ...
    >
      {/* responsive container - desktop üçün row, mobile üçün column */}
      <div className="relative flex flex-col md:flex-row p-4 bg-[#131313] rounded-lg w-full max-w-2xl">
        ...
        {/* image seçmə qutusu */}
        <div
          className="w-32 h-32 md:w-40 md:h-40 bg-zinc-800 flex items-center justify-center rounded-md cursor-pointer overflow-hidden self-center md:self-start mt-2"
        ...
        {/* sağ tərəfdə form - mobile-da altda görünür */}
        <div className="flex flex-col flex-1 space-y-3 mt-4 md:mt-0 md:ml-5">
          ...
          <button
            onClick={handleSave}
            className="bg-white text-black rounded-2xl p-2 font-bold hover:bg-gray-300 w-full md:w-1/3"
          >
            Save
          </button>
        </div>
